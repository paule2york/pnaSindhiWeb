// Free translation to Sindhi (sd), with a permanent Supabase cache.
// Provider order: Azure (Microsoft Translator) first when configured, then
// several free Google-backed endpoints as fallback. Every successful result is
// cached in Supabase, so each unique phrase is translated only ONCE (ever).
//
// Optional env vars (set in Vercel) to enable Azure:
//   AZURE_TRANSLATOR_KEY      - your Translator resource key
//   AZURE_TRANSLATOR_REGION   - resource region, e.g. "eastus" (required for
//                               regional/multi-service resources)
...
// ---- Provider 0: Azure / Microsoft Translator (primary, if configured) ----
...
// ---- Providers 1-3: free Google-backed endpoints (fallback) ----
...
const PROVIDERS = [viaAzure, viaGoogleGtx, viaGoogleClients5, viaLingva];
