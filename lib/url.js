...
// Uses a short numeric id so links stay clean.
// Also embeds base64-encoded source URL as ?u= so the article page can
// always resolve the link without needing Supabase or an RSS fallback.
export function articlePath(item = {}) {
  if (!item || item.source === 'local' || !item.link) return '/';
  const token = shortId(item.link);
  const b64 = encodeUrl(item.link);
  const q = item.native ? `?n=1&u=${b64}` : `?u=${b64}`;
  return `/article/${token}${q}`;
}
