...
// Uses base64-encoded source URL so the article page can always decode it
// without needing Supabase (a numeric shortId is too short to decode).
export function articlePath(item = {}) {
  if (!item || item.source === 'local' || !item.link) return '/';
  const token = encodeUrl(item.link);
