...
const NATIVE_SINDHI_NEWS = [
  // Verified active RSS feeds (20 Jul 2026). Awami Awaz intentionally excluded:
  // its article thumbnails are publication-logo placeholders rather than news photos.
  { url: 'https://dailyibrat.com/?feed=rss2', source: 'عبرت' },
...
export const SOURCES = [
  ...
  'عبرت', 'سنڌ ايڪسپريس', 'سنڌ ٽي وي نيوز', 'ڪي ٽي اين نيوز', 'سنڌ ميٽرز',
];
...
const SINDH_KEYWORDS = [
  ...
  'indus', 'mohenjo', 'kotdiji', 'gorakh', 'keenjhar', 'manchar',
  ...
  'ٽنڊو', 'لياري', 'ملير', 'سنڌودير', 'مهراڻ', 'موئن جو دڑو', 'منڇر', 'ڪينجهر',
];

function isSindhRelated(it) {
  const hay = (
    String(it.rawTitle || '') + ' ' + String(it.rawDescription || '') + ' ' + String(it.link || '')
  ).toLowerCase();
  return SINDH_KEYWORDS.some((k) => hay.includes(k));
}
...
export async function fetchFeedNews(category, limit = 12) {
  ...
  // The 'sindh' category pulls from general Pakistan feeds, so keep only items
  // that actually mention Sindh / its cities. Native Sindhi-language sources
  // (e.g. Ibrat) are Sindh-focused papers, so they pass through as-is.
  if (category === 'sindh') {
    all = all.filter((it) => it.native || isSindhRelated(it));
  }
  ...
  // Rewrite native Sindhi items so they are original (not copy).
  const nativeItems = unique.filter((it) => it.native);
  if (nativeItems.length) {
    const origTitles = nativeItems.map((it) => it.rawTitle);
    const origDescs = nativeItems.map((it) => it.rawDescription);
    const rewritesT = await Promise.all(origTitles.map((t) => toSindhiRewrite(t)));
    const rewritesD = await Promise.all(origDescs.map((d) => toSindhiRewrite(d)));
    nativeItems.forEach((it, i) => {
